...
async function checkAuth() {
  try {
    ...
    if (data.isAuthenticated) {
      ...
      if (document.getElementById('admin-link')) {
        if (data.user.is_admin || data.user.is_super_admin || data.user.id === 1) {
          document.getElementById('admin-link').innerHTML = '<a href="admin.html" style="color:#c2a469;">后台管理</a>';
        } else {
          document.getElementById('admin-link').innerHTML = '';
        }
      }
...
function renderFriendRequestEntry() {
  ...
  if (!entry) {
    ...
    entry.style = 'position:fixed;bottom:30px;right:30px;z-index:9999;background:#c2a469;color:#fff;border:none;padding:0.7rem 1.2rem;border-radius:8px;box-shadow:0 2px 8px rgba(194,164,105,0.12);cursor:pointer;font-size:1rem;';
