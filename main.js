...
// ========== 临界阻尼动画工具 ==========
/**
 * 临界阻尼平滑动画（用于弹窗、Tab切换、消息滚动等）
 * @param {HTMLElement} el 目标元素
 * @param {string} prop CSS属性（如'opacity'）
 * @param {number} target 目标值
 * @param {object} opts {gamma, omega0, duration}
 */
function smoothDampedAnimate(el, prop, target, opts = {}) {
  const gamma = opts.gamma || 0.6;
  const omega0 = opts.omega0 || 1.2;
  const duration = opts.duration || 300;
  let v = 0, x = parseFloat(el.style[prop]) || 0;
  let start = null;
  function animate(ts) {
    if (!start) start = ts;
    const dt = 1 / 60;
    ...
    v += a * dt;
    x += v * dt;
    el.style[prop] = Math.max(0, Math.min(x, 1));
    if ((target - x) * (target - (x + v * dt)) > 0 && ts - start < duration) {
 ...
 * @param {number} lastNorm 上一次范数
 * @param {number} threshold 收敛阈值
 * @param {function} renderFn 渲染函数
 * @param {function} [extraJudge] 额外终止判据 (node, depth, currentNorm) => boolean
 */
function recursiveAdaptiveLoad(node, depth, maxDepth, lastNorm, threshold, renderFn, extraJudge) {
  if (depth > maxDepth) return;
  const currentNorm = Array.isArray(node.children) ? node.children.length : 0;
  if (Math.abs(currentNorm - lastNorm) < threshold) return;
  if (extraJudge && !extraJudge(node, depth, currentNorm)) return;
  renderFn(node, depth);
  if (Array.isArray(node.children)) {
    node.children.forEach(child =>
      recursiveAdaptiveLoad(child, depth + 1, maxDepth, currentNorm, threshold, renderFn, extraJudge)
    );
  }
}
...
// 聊天消息渲染（带去重，批量渲染时移除收敛条件提前终止，完整渲染所有消息）
function appendMessage(msg, prepend = false) {
  ...
  // 支持图片消息和@高亮
  let content = escapeHtml(msg.content || msg.message || '');
  // 图片消息支持（简单实现：以[img]url[/img]格式发送）
  ...
  // @高亮
  content = content.replace(/@([a-zA-Z0-9_]{3,16})/g, '<span style="color:#c2a469;font-weight:bold;">@$1</span>');
...
// ========== 前端骨架屏/加载动画自适应优化 ==========
function showSkeletonLoader(container, duration = 800) {
  if (!container) return;
  const skeleton = document.createElement('div');
  skeleton.className = 'skeleton-loader';
  skeleton.style = 'height:2em;background:linear-gradient(90deg,#f3e9d7 25%,#fdf6e3 50%,#f3e9d7 75%);background-size:200% 100%;animation:skeleton-loading 1.2s infinite linear;';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  showSkeletonLoader(mainContainer, 600 + Math.random() * 400); // 响应时间分布的熵自适应
  try {
    // 先尝试获取CSRF令牌
    const token = await getCsrfToken();
