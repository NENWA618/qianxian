...
function renderNotificationCenterList(tab = 'friend') {
  ...
  if (tab === 'friend') {
    ...
    // 系统通知（可扩展，暂时占位）
    container.innerHTML = `<div style="color:#aaa;text-align:center;">暂无系统通知</div>`;
  }
}
...
// 聊天未读红点（简单实现：可根据后端未读消息数扩展）
function updateChatRedDots() {
  // 这里可根据后端返回的未读消息数，前端暂时不做复杂处理
  // 示例：假设有未读消息则显示红点
  // 可扩展为维护一个unreadChat对象，key为userId，value为未读数
}
