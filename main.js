...
function showPasswordStrength(password, container) {
  ...
  let color = '';
  switch (score) {
    case 0: text = '极弱'; color = '#c62828'; break;
    case 1: text = '较弱'; color = '#e57373'; break;
    case 2: text = '一般'; color = '#fbc02d'; break;
    case 3: text = '较强'; color = '#388e3c'; break;
    case 4: text = '极强'; color = '#2e7d32'; break;
...
function validateInput(str, type = "text") {
  ...
  if (type === "username") {
    return /^[a-zA-Z0-9_\u4e00-\u9fa5]{2,32}$/.test(str);
  }
  if (type === "password") {
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (res.status === 403) {
      showGlobalError('登录失败：权限不足或CSRF校验失败，请刷新页面重试');
      msg.textContent = '登录失败：权限不足或CSRF校验失败';
      msg.style.color = 'red';
      return;
    }
    if (!res.ok) {
      showGlobalError('登录失败：用户名或密码错误');
      msg.textContent = '登录失败：用户名或密码错误';
      msg.style.color = 'red';
      return;
    }
    const data = await res.json();
...
document.getElementById("register-form").addEventListener("submit", async (e) => {
  ...
  if (!validateInput(username, "username")) {
    showMessage(messageElement, "用户名格式不合法，仅允许3-32位字母、数字、下划线", "red");
    return;
  }
