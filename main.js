    ...
    if (data.success && Array.isArray(data.messages)) {
      if (data.messages.length > 0) {
        ...
        for (let i = 0; i < data.messages.length; i++) {
          ...
          // 收敛条件：消息内容长度变化很小时提前break
          const norm = Math.abs((msg.content || '').length - ((data.messages[i - 1]?.content || '').length));
          if (i > 0 && Math.log((norm + 1) / (lastNorm + 1)) < delta) {
            breakFlag = true;
            break;
          }
          lastNorm = norm;
