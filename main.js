...
(async function loadDynamicParams() {
  try {
    const params = await fetchDynamicParams();
...
function renderAdaptiveInfoPanel(arr, panelId = "adaptive-info-panel") {
  ...
  if (!panel) return;
  // 计算分形维度、熵、方差等
  const Df = boxCountingDimension(arr.map(x => typeof x === "number" ? x : (x.id || 0)));
  const ent = entropy(arr.map(x => typeof x === "number" ? x : (x.id || 0)));
  const varr = variance(arr.map(x => typeof x === "number" ? x : (x.id || 0)));
  const kurt = kurtosis(arr.map(x => typeof x === "number" ? x : (x.id || 0)));
  // 推荐分页粒度
  const pageSize = recursiveNcrit(
    arr.map(x => typeof x === "number" ? x : (x.id || 0)),
    (a) => entropy(a),
    (window.dynamicParams.ncrit && window.dynamicParams.ncrit.maxDepth) || 5,
    (window.dynamicParams.ncrit && window.dynamicParams.ncrit.delta) || 2,
    Df
  );
  panel.innerHTML = `
    <div>
      <strong>分形维度 D<sub>f</sub>：</strong> ${Df.toFixed(2)}
      <span style="margin-left:1.5em;"><strong>信息熵：</strong> ${ent.toFixed(2)}</span>
      <span style="margin-left:1.5em;"><strong>方差：</strong> ${varr.toFixed(2)}</span>
      <span style="margin-left:1.5em;"><strong>峰度：</strong> ${kurt.toFixed(2)}</span>
      <span style="margin-left:1.5em;"><strong>推荐分页粒度：</strong> ${pageSize}</span>
    </div>
  `;
  panel.style.display = '';
}
...
function showPasswordStrength(password, container) {
  if (!window.zxcvbn || typeof window.zxcvbn !== "function") return;
  const result = zxcvbn(password);
  const score = result.score;
  const feedback = result.feedback.suggestions.join(" ");
  let color = ["#e57373", "#ffb74d", "#fff176", "#81c784", "#388e3c"][score];
  container.innerHTML = `
    <div style="height:8px;background:${color};border-radius:4px;margin-bottom:4px;width:${(score + 1) * 20}%"></div>
    <div style="font-size:13px;color:#888;">${feedback || ["极弱", "弱", "一般", "强", "极强"][score]}</div>
  `;
}
