...
function connectChat() {
  ...
  socket.on('chat message', (data) => {
    ...
    if (
      ...
      (
        // 自己发的，或对方发给自己
        (Number(data.senderId || data.sender_id) === Number(currentUser.id) && Number(data.targetId || data.target_id) === Number(currentTargetId)) ||
        (Number(data.senderId || data.sender_id) === Number(currentTargetId) && Number(data.targetId || data.target_id) === Number(currentUser.id))
      )
    ) {
      appendMessage(data, false);
      loadUnreadChat();
