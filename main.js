...
// ====== 修复：全局showMessage函数 ======
function showMessage(elementOrMsg, msg, color = 'red') {
  // 支持 showMessage('xxx') 或 showMessage(dom, 'xxx', 'red')
  if (typeof elementOrMsg === 'string' && typeof msg === 'undefined') {
    showGlobalError(elementOrMsg);
    return;
  }
  if (elementOrMsg && typeof elementOrMsg === 'object' && 'textContent' in elementOrMsg) {
    elementOrMsg.textContent = msg;
    elementOrMsg.style.color = color;
  } else {
    showGlobalError(msg || elementOrMsg);
  }
}

// ====== 修复：removeEditHomeContentPanel 占位函数 ======
...
// ====== 修复：showEditHomeContentPanel 占位函数 ======
function showEditHomeContentPanel() {
  // 如果需要显示首页内容编辑面板，可在此实现
  // 这里只是占位，防止未定义报错
}

// ====== 修复：updateHomeSections 占位函数 ======
function updateHomeSections() {
  // 占位，防止未定义报错。后续可补充首页内容渲染逻辑。
}
...
function renderAdaptiveInfoPanel(arr, panelId = "adaptive-info-panel") {
  ...
  const H = -arr.map(x => x / sum).filter(p => p > 0).reduce((a, p) => a + p * Math.log(p), 0);
  const minVal = Math.min(...arr);
  const maxVal = Math.max(...arr);
  const range = maxVal - minVal;
  let Df = 1;
  if (range > 0) {
    Df = boxCountingDimension(arr);
  }
  panel.innerHTML = `<strong>自适应参数：</strong> 信息熵 H=${H.toFixed(3)}，分形维度 Df=${Df.toFixed(3)}`;
  panel.style.display = '';
}
...
function showPasswordStrength(password, container) {
  ...
  let color = '';
  switch (score) {
    case 0: text = '极弱'; color = '#d32f2f'; break;
    case 1: text = '较弱'; color = '#f57c00'; break;
    case 2: text = '一般'; color = '#fbc02d'; break;
    case 3: text = '较强'; color = '#388e3c'; break;
    case 4: text = '极强'; color = '#1976d2'; break;
    default: text = '未知'; color = '#888';
  }
  container.textContent = `密码强度：${text}`;
...
function validateInput(str, type = "text") {
  ...
  if (type === "username") {
    return /^[a-zA-Z0-9_\u4e00-\u9fa5]{3,16}$/.test(str);
  }
  if (type === "password") {
    return str.length >= 8 && !/[<>"';]/.test(str);
  }
...
function appendMessage(msg, prepend = false) {
  ...
  if (prepend) {
    ...
    autoScrollChatToBottom(chatMessages);
  }
}

// ========== 公聊/私聊统一自动滚动到底部 ==========
function autoScrollChatToBottom(container) {
  // 判断用户是否在底部（如需强制始终滚动到底部可去掉此判断）
  const isAtBottom = Math.abs(container.scrollHeight - container.clientHeight - container.scrollTop) < 10;
  if (!isAtBottom && container.scrollHeight > container.clientHeight + 50) {
    // 用户手动滚动离开底部时不自动滚动
    return;
  }
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  if (!validateInput(username, "username") || !validateInput(password, "password")) {
    msgDiv.textContent = '用户名或密码格式不正确';
    msgDiv.style.color = 'red';
    return;
  }
  try {
    ...
    if (res.status === 403) {
      showGlobalError('登录已过期或CSRF失效，请刷新页面');
      msgDiv.textContent = '登录已过期或CSRF失效，请刷新页面';
      msgDiv.style.color = 'red';
      resetCsrfToken();
      return;
    }
    const data = await res.json();
    if (data.success) {
      resetCsrfToken(); // 登录成功后重置CSRF
      msgDiv.textContent = '登录成功';
      msgDiv.style.color = 'green';
      // 登录成功后的处理，如刷新页面或加载用户信息
      window.location.reload();
    } else {
      msgDiv.textContent = data.message || '登录失败';
      msgDiv.style.color = 'red';
      showGlobalError(data.message || '登录失败');
    }
  } catch (err) {
    msgDiv.textContent = '网络错误，请重试';
    msgDiv.style.color = 'red';
    showGlobalError('登录失败，请检查网络');
  }
});
...
document.getElementById('logout-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.success) {
      resetCsrfToken();
      msgDiv.textContent = '已退出登录';
      msgDiv.style.color = 'green';
      // 登录成功后的处理，如刷新页面或加载用户信息
      window.location.reload();
    } else {
      msgDiv.textContent = data.message || '退出登录失败';
      msgDiv.style.color = 'red';
      showGlobalError(data.message || '退出登录失败');
    }
  } catch (err) {
    msgDiv.textContent = '网络错误，请重试';
    msgDiv.style.color = 'red';
    showGlobalError('退出登录失败，请检查网络');
  }
});
...
document.getElementById("register-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (res.status === 403) {
      showGlobalError('注册已过期或CSRF失效，请刷新页面');
      msgDiv.textContent = '注册已过期或CSRF失效，请刷新页面';
      msgDiv.style.color = 'red';
      resetCsrfToken();
      return;
    }
    ...
    if (data.success) {
      ...
      msgDiv.style.color = 'red';
      showGlobalError(data.message || '注册失败');
    }
  } catch (err) {
    msgDiv.textContent = '网络错误，请重试';
    msgDiv.style.color = 'red';
    showGlobalError('注册失败，请检查网络');
  }
});

// =================== 其它敏感操作fetch请求示例 ===================
// 例如：修改密码、发送消息、好友申请等
// 只要是POST/DELETE等敏感操作，都要用getCsrfToken()获取token，并加到header
...
// main.js - 登录/注册/敏感操作等全局前端逻辑（CSRF安全增强版）
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // 2. 发起登录请求，带上token和credentials
        const res = await fetch((window.API_BASE_URL || 'https://qianxian-backend.onrender.com') + '/api/login', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (res.ok && data.success) {
          ...
          // 登录失败也建议重置CSRF Token，防止token/session不同步
          window.resetCsrfToken();
        }
      } catch (err) {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        const csrf = await window.getCsrfToken();
        const res = await fetch((window.API_BASE_URL || 'https://qianxian-backend.onrender.com') + '/api/register', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async () => {
      try {
        const csrf = await window.getCsrfToken();
        const res = await fetch((window.API_BASE_URL || 'https://qianxian-backend.onrender.com') + '/api/logout', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  if (changePwdForm) {
    changePwdForm.addEventListener('submit', async (e) => {
      ...
      try {
        const csrf = await window.getCsrfToken();
        const res = await fetch((window.API_BASE_URL || 'https://qianxian-backend.onrender.com') + '/api/change-password', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  // 其他敏感操作（如API Token管理、资料修改等）也应采用同样的CSRF安全增强逻辑
});