...
(async function loadDynamicParams() {
  try {
    ...
    if (data.success) window.dynamicParams = data.params || {};
  } catch (e) {
    window.dynamicParams = {};
  }
})();
...
function renderAdaptiveInfoPanel(arr, panelId = "adaptive-info-panel") {
  ...
  let Df = 1;
  if (range > 0) {
    const scaledArr = arr.map(x => Math.floor(((x - minVal) / range) * 10));
    const boxCounts = new Set(scaledArr);
    Df = Math.log(boxCounts.size) / Math.log(10);
  }
...
function showPasswordStrength(password, container) {
  ...
  let color = '';
  switch (score) {
    case 0: text = '极弱'; color = '#e74c3c'; break;
    case 1: text = '较弱'; color = '#e67e22'; break;
    case 2: text = '一般'; color = '#f1c40f'; break;
    case 3: text = '较强'; color = '#2ecc71'; break;
    case 4: text = '极强'; color = '#27ae60'; break;
    default: text = '未知'; color = '#aaa';
  }
...
function validateInput(str, type = "text") {
  ...
  if (type === "username") {
    // 用户名允许中英文、数字、下划线、2-32位
    return /^[\u4e00-\u9fa5\w]{2,32}$/.test(str);
  }
  if (type === "password") {
    // 密码最少8位
    return str.length >= 8;
