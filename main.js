  ...
  try {
    ...
    if (data.success && Array.isArray(data.messages)) {
      if (data.messages.length > 0) {
        ...
        const entropy = intervals.length > 0 ? -intervals.map(x => x / (intervals.reduce((a, b) => a + b, 0) || 1)).filter(p => p > 0).reduce((a, p) => a + p * Math.log(p), 0) : 0;
        // 若熵较低（消息分布集中），批量渲染速度加快
        let batchSize = 10 + Math.round(entropy * 10);
        batchSize = Math.max(5, Math.min(batchSize, 30));
        for (let i = 0; i < data.messages.length; i += batchSize) {
          const batch = data.messages.slice(i, i + batchSize);
          batch.forEach(msg => appendMessage(msg, !!beforeId));
        }
