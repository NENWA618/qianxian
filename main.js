...
function connectChat() {
  ...
  socket.on('chat message', (data) => {
    ...
    // data 结构应包含 id, username, is_admin, is_super_admin, message, time
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      await checkAuth(); // 更新用户状态
    } else {
      showMessage(messageElement, `登录失败: ${data.message}`, 'red');
    }
  } catch (err) {
    showMessage(messageElement, `错误: ${err.message}`, 'red');
  }
});
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      if (error.message.includes('CSRF')) {
        console.log('CSRF令牌失效，重试中...');
        return makeAuthenticatedRequest(url, method, body); // 自动重试
      }
    }
...
// 刷新首页 about/announcement section 内容
  ...
  try {
    ...
    if (data.success) {
      if (document.getElementById('about-section')) {
        document.getElementById('about-section').innerHTML = data.content.about || '';
      }
      if (document.getElementById('announcement-section')) {
        document.getElementById('announcement-section').innerHTML = data.content.announcement || '';
      }
    }
  } catch (err) {
    // 忽略
