...
(async function loadDynamicParams() {
  try {
    ...
    if (res.ok) {
      window.dynamicParams = await res.json();
    }
  } catch (e) {}
})();
...
function renderAdaptiveInfoPanel(arr, panelId = "adaptive-info-panel") {
  ...
  const range = max - min;
  const boxSize = Math.ceil(range / 10) || 1;
  const boxes = new Set();
  for (const v of arr) {
    boxes.add(Math.floor((v - min) / boxSize));
  }
  const Df = Math.log(boxes.size) / Math.log(10);
  // 计算熵
  ...
  panel.innerHTML = `
    <div>
      <strong>分形维度 D<sub>f</sub>：</strong> ${Df.toFixed(2)}<br>
      <strong>信息熵：</strong> ${entropy.toFixed(2)}<br>
      <strong>递归终止 N<sub>crit</sub>：</strong> ${ncrit}
    </div>
  `;
  panel.style.display = '';
}
...
function appendMessage(msg, prepend = false) {
  ...
  const user = {
    id: msg.sender_id || msg.senderId || msg.id,
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      setTimeout(() => {
        handleLoginSuccess();
        // window.location.reload(); // 可选：如需刷新页面
      }, 500);
      return;
    } else {
      const errorMsg = data.message || '未知错误';
...
async function checkAuth() {
  try {
    ...
    if (!token) {
      ...
      // 移除聊天小窗
      const chatFloat = document.getElementById('chat-float-iframe');
      if (chatFloat) chatFloat.remove();
      return;
    }
