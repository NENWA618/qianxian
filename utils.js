...
function recursiveNcrit(arr, judgeFn, maxDepth = 5, delta = 2, fractalDimension) {
  // 支持参数热更新：从后端获取ncrit.delta、ncrit.maxDepth
  let ncritDelta = delta;
  let ncritMaxDepth = maxDepth;
  if (window.dynamicParams && typeof window.dynamicParams.ncrit === "object") {
    if (typeof window.dynamicParams.ncrit.delta === "number") ncritDelta = window.dynamicParams.ncrit.delta;
    if (typeof window.dynamicParams.ncrit.maxDepth === "number") ncritMaxDepth = window.dynamicParams.ncrit.maxDepth;
  }
  // 分形维度 Df 参与自适应
  ...
  let fractalFactor = 1;
  if (Df > 1) {
    fractalFactor = Math.max(0.5, 2.5 - Df);
  }
  let prev = arr.length;
  let ncrit = 0;
  let pageSize = prev;
