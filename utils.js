...
async function getCsrfToken(retryCount = 0) {
  ...
  try {
    ...
    const token = data.token || data.csrfToken; // 兼容后端返回的不同字段
    if (!token) throw new Error('获取CSRF令牌失败');
    csrfToken = token;
...
/**
 * 清空CSRF Token（如登录/登出后调用，避免token失效）
 */
 ...
 * @param {number} [fractalDimension] - 可选，分形维度Df
 * @returns {number} - 推荐分页粒度
 */
function recursiveNcrit(arr, judgeFn, maxDepth = 5, delta = 2, fractalDimension) {
  // 支持参数热更新：从后端获取ncritDelta、ncritMaxDepth
  let ncritDelta = delta;
  let ncritMaxDepth = maxDepth;
  if (window.dynamicParams && typeof window.dynamicParams === "object") {
    if (typeof window.dynamicParams.ncritDelta === "number") ncritDelta = window.dynamicParams.ncritDelta;
    if (typeof window.dynamicParams.ncritMaxDepth === "number") ncritMaxDepth = window.dynamicParams.ncritMaxDepth;
  }
  // 分形维度 Df 参与自适应
  let Df = typeof fractalDimension === "number" ? fractalDimension : boxCountingDimension(arr);
  // Df 越大，建议分页粒度越小（更细分）
  let fractalFactor = 1;
  if (Df > 1) {
    fractalFactor = Math.max(0.5, 2.5 - Df); // Df=1.6时约0.9，Df=2.2时约0.3
  }
  let prev = arr.length;
  let ncrit = 0;
  let pageSize = prev;
  for (let i = 0; i < ncritMaxDepth; i++) {
    const val = judgeFn(arr);
    let newPageSize = Math.round(10 + val * 10 * fractalFactor + i * 2);
    newPageSize = Math.min(Math.max(1, newPageSize), 100);
    if (Math.abs(newPageSize - prev) < ncritDelta) {
      ncrit++;
    } else {
      ncrit = 0;
    }
 ...
 * @param {object} options - { duration, onUpdate, onComplete, gammaC }
 */
function smoothDampedAnimate(el, prop, target, options = {}) {
  ...
  const dt = 1 / fps;
  // 支持热更新gammaC参数
  let gamma = 2 * Math.sqrt(1);
  if (options.gammaC) gamma = options.gammaC;
  else if (window.dynamicParams && typeof window.dynamicParams.gammaC === "number") gamma = window.dynamicParams.gammaC;
  const omega0 = 1;
...
// ========== 新增：前端动态参数快照获取 ==========
/**
 * 获取后端动态参数快照（用于前端自适应/热更新）
 * @returns {Promise<object>}
 */
async function fetchDynamicParams() {
  const API_BASE_URL = window.API_BASE_URL || 'https://qianxian-backend.onrender.com';
  try {
    const res = await fetch(API_BASE_URL + '/api/dynamic-params', { credentials: 'include' });
    if (!res.ok) throw new Error('获取动态参数失败');
    const data = await res.json();
    if (data && typeof data === "object") {
      window.dynamicParams = data;
      return data;
    }
    return {};
  } catch (e) {
...
window.fetchDynamicParams = fetchDynamicParams;