...
async function fetchTokens() {
  try {
    ...
    if (res.status === 403) {
      ...
      return { success: false, message: '登录已过期或CSRF失效，请刷新页面' };
    }
    if (!res.ok) throw new Error('获取Token列表失败');
    return await res.json();
  } catch (err) {
    showGlobalError('Token加载失败，请检查网络');
    return { success: false, message: 'Token加载失败，请检查网络' };
  }
}
...
async function createToken(desc, expire) {
  try {
    ...
    return { success: false, message: 'Token创建失败，请检查网络' };
  }
}
...
async function deactivateToken(tokenId) {
  try {
    ...
    showGlobalError('Token失效失败，请检查网络');
    return { success: false, message: 'Token失效失败，请检查网络' };
  }
}
