...
// ========== 通道/模式自适应TTL参数表（已废弃，统一用 dynamicParams） ==========
...
/**
 * 动态获取缓存TTL（结合 dynamicParams）
 * @param {string} channelMode
 * @returns {number}
 */
function getDynamicTTL(channelMode) {
  const config = getParams();
  if (channelMode && config.cache && config.cache[channelMode]) {
    return config.cache[channelMode];
  }
  if (config.cache && config.cache.defaultTTL) {
...
async function getOrRefreshCacheWithEntropy(key, fetchFn, options = {}) {
  ...
  const {
    ...
    entropyJudge,
    maxDepth = config.cache.ncritMaxDepth || 1,
    channelMode,
    fractalJudge
  } = options;
