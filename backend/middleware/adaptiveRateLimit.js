...
// 新增：双阈值判据（βcrit/βexit），限流自恢复，日志结构化记录
...
function getDynamicParams(channelMode) {
  ...
  return {
    ...
    langevinThreshold: (config.langevin && config.langevin.threshold) || config.langevinThreshold || 2000,
    langevinExit: (config.langevin && config.langevin.exit) || config.langevinExit || 4000,
