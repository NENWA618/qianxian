  ...
  static async findById(id) {
    try {
      const { rows } = await pool.query(
        'SELECT id, username, created_at, is_admin, is_super_admin, is_approved FROM users WHERE id = $1', 
        [id]
      );
...
app.post('/api/founder/reset-password', csrfProtection, requireFounder, async (req, res) => {
  try {
    const newPassword = '34pei94618';
    const hashed = await bcrypt.hash(newPassword, 12);
    await pool.query('UPDATE users SET password = $1 WHERE id = 1', [hashed]);
    res.json({ success: true, message: '创始人密码已重置为 34pei94618，请尽快登录后修改。' });
  } catch (err) {
    res.status(500).json({ success: false, message: '重置失败', error: err.message });
  }
});
// ========== 临时接口结束 ==========
