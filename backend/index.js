...
// ========== 递归熵驱动缓存刷新优化（重构：使用cache.js工具） ==========
async function getSiteContentWithEntropy() {
  // 优先查缓存
  let content = await cache.getCache('site_content');
  ...
  // 统计访问分布（可用作熵判据）
  const now = Date.now();
  const accessArr = rows.map((row, i) => now - i * 1000); // 模拟分布
  ...
  await cache.setCache('site_content', content, 300, { entropyArr: accessArr, threshold: 0.02 });
  return content;
}
// ========== END 递归熵驱动缓存刷新 ==========
...
app.post('/api/site-content', csrfProtection, async (req, res) => {
  ...
  try {
    ...
    // 清理缓存
    await cache.delCache('site_content');
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/members', async (req, res) => {
  try {
    ...
    if (req.query.autoPageSize === '1') {
      ...
      const entropy = cache.entropy(ages);
...
async function logOperation({ userId, action, targetId, detail }) {
  try {
    await pool.query(
      `INSERT INTO operation_logs (user_id, action, target_id, detail, created_at)
       VALUES ($1, $2, $3, $4, NOW())`,
...
// 在权限变更、审核等敏感操作处调用 logOperation
// 例如在 set-admin、set-super-admin、approve-user 路由中：
// await logOperation({ userId: req.user.id, action: 'set-admin', targetId: userId, detail: `isAdmin=${isAdmin}` });
...
async function initDB() {
  try {
    ...
    `);
    // 新增：确保is_protected和protected_by字段
...
const upload = multer({ dest: path.join(__dirname, 'uploads/') }); // 可根据需要调整
