...
// --- 新增：好友关系API ---
app.post('/api/friends/add', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated()) return res.status(401).json({ success: false, message: '未登录' });
  const { friendId } = req.body;
  if (Number(friendId) === req.user.id) return res.status(400).json({ success: false, message: '不能添加自己为好友' });
  try {
    await pool.query(
...
app.get('/api/chat/messages', async (req, res) => {
  ...
  if (targetType === 'public') {
    sql = `SELECT m.*, u.username, u.is_admin, u.is_super_admin FROM chat_messages m JOIN users u ON m.sender_id = u.id WHERE m.target_type = 'public' ORDER BY m.created_at ASC LIMIT 100`;
    params = [];
  } else if (targetType === 'private' && targetId) {
    sql = `SELECT m.*, u.username, u.is_admin, u.is_super_admin FROM chat_messages m JOIN users u ON m.sender_id = u.id WHERE m.target_type = 'private' AND ((m.sender_id = $1 AND m.target_id = $2) OR (m.sender_id = $2 AND m.target_id = $1)) ORDER BY m.created_at ASC LIMIT 100`;
  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS chat_messages (
        ...
        target_type VARCHAR(16) NOT NULL, -- 'public' 或 'private'
        target_id INTEGER,
...
io.on('connection', (socket) => {
  ...
  socket.on('chat message', async (msg) => {
    ...
    try {
      await pool.query(
        `INSERT INTO chat_messages (sender_id, target_type, target_id, content) VALUES ($1, $2, $3, $4)`,
