...
async function getSiteContentWithEntropy() {
  ...
  rows.forEach(row => {
    ...
    block.accessLog.push(Date.now());
    // 只保留最近100次访问
    if (block.accessLog.length > 100) block.accessLog.shift();
    // 计算访问分布熵
    const intervals = [];
    for (let i = 1; i < block.accessLog.length; i++) {
      intervals.push(block.accessLog[i] - block.accessLog[i - 1]);
    }
    let entropy = 0;
    ...
    // 新增：熵变化速率
    const deltaH = entropy - (block.lastEntropy || 0);
    const deltaT = (Date.now() - (block.lastUpdate || 0)) / 1000;
    const dHdt = deltaT > 0 ? deltaH / deltaT : 0;
    // 临界阻尼联合判据
    const gamma = 0.7, omega0 = 1.5;
    const threshold = 0.2 + Math.abs(dHdt) * gamma / omega0;
    if (
      Math.abs(entropy - (block.lastEntropy || 0)) > threshold ||
      Date.now() - (block.lastUpdate || 0) > 1000 * 60 * 5
    ) {
      block.value = row.value;
      block.lastUpdate = Date.now();
      block.lastEntropy = entropy;
      needRefresh = true;
    }
  });
  // 返回所有内容块
