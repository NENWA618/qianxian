// ...existing code...临时迁移脚本
if (process.env.RUN_MIGRATION === '1') {
  (async () => {
    // 例如：为 site_content 表增加 entropy 字段
    await pool.query(`ALTER TABLE site_content ADD COLUMN IF NOT EXISTS entropy FLOAT DEFAULT 0`);
    console.log('迁移完成');
    process.exit(0);
  })();
}
...
async function getSiteContentWithEntropy() {
  ...
  rows.forEach(row => {
    ...
    // 若熵变化大于阈值或5分钟未刷新则刷新
    if (
      Math.abs(entropy - block.lastEntropy) > 0.2 ||
      Date.now() - block.lastUpdate > 1000 * 60 * 5
    ) {
