...
// 1. 数据库迁移：users表增加is_protected字段
async function ensureIsProtectedColumn() {
  try {
    await pool.query(`
      ALTER TABLE users ADD COLUMN IF NOT EXISTS is_protected BOOLEAN DEFAULT FALSE
    `);
    console.log('已确保users表存在is_protected字段');
  } catch (err) {
    console.error('添加is_protected字段失败:', err);
  }
}
...
app.post('/api/admin/set-protected', csrfProtection, requireSuperAdminOrAdmin, async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE users SET is_protected=$1 WHERE id=$2', [!!isProtected, userId]);
    await logOperation({
      userId: req.user.id,
      action: 'set-protected',
      targetId: userId,
      detail: `isProtected=${!!isProtected}`
    });
    res.json({ success: true });
  } catch (err) {
  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS users (
        ...
        is_protected BOOLEAN DEFAULT FALSE -- 新增字段
      )
    `);
    await pool.query(`
