...
const { getParams, setParam, setParamsBatch, getChangeLog, resetParams, canChangeParams } = require('./config/dynamicParams');
...
// ========== 使用自适应速率限制（论文算法应用点：通道/模式/用户类型自适应） ==========
// 多通道负载分流：按路由类型区分限流参数
app.use((req, res, next) => {
  // 由adaptiveRateLimit内部自动按通道/模式/用户类型自适应，无需外部细分
  return adaptiveRateLimit({ criticalDamping: true })(req, res, next);
});
...
// ========== 动态参数管理API ==========
// 获取所有动态参数（前端自适应/热更新用）
app.get('/api/dynamic-params', (req, res) => {
  res.json(getParams(true));
});
// 管理员接口
app.get('/api/admin/params', (req, res) => {
  if (!req.isAuthenticated() || !(req.user.is_admin || req.user.is_super_admin)) {
    return res.status(403).json({ success: false, message: '无权限' });
  }
  res.json({ success: true, params: getParams(), changeLog: getChangeLog() });
});
app.get('/api/admin/params/log', (req, res) => {
  if (!req.isAuthenticated() || !(req.user.is_admin || req.user.is_super_admin)) {
    return res.status(403).json({ success: false, message: '无权限' });
  }
  res.json({ success: true, changeLog: getChangeLog() });
});
app.post('/api/admin/params', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated() || !(req.user.is_admin || req.user.is_super_admin)) {
    return res.status(403).json({ success: false, message: '无权限' });
  }
  ...
  try {
    setParam(key, value, req.user);
    res.json({ success: true, params: getParams(), changeLog: getChangeLog() });
  } catch (err) {
    res.status(403).json({ success: false, message: err.message || '参数变更失败' });
  }
});
app.post('/api/admin/params/batch', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated() || !(req.user.is_admin || req.user.is_super_admin)) {
    return res.status(403).json({ success: false, message: '无权限' });
  }
  const { updates } = req.body;
  if (!updates || typeof updates !== "object") return res.status(400).json({ success: false, message: '缺少参数updates' });
  try {
    setParamsBatch(updates, req.user);
    res.json({ success: true, params: getParams(), changeLog: getChangeLog() });
  } catch (err) {
    res.status(403).json({ success: false, message: err.message || '批量参数变更失败' });
  }
});
app.post('/api/admin/params/reset', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated() || !(req.user.is_admin || req.user.is_super_admin)) {
    return res.status(403).json({ success: false, message: '无权限' });
  }
  try {
    resetParams(req.user);
    res.json({ success: true, params: getParams(), changeLog: getChangeLog() });
  } catch (err) {
    res.status(403).json({ success: false, message: err.message || '参数重置失败' });
  }
});

// ========== 缓存监控API ==========
app.get('/api/monitor/cache', (req, res) => {
  res.json({ success: true, cache: monitorCache() });
});
...
app.get('/api/csrf-token', (req, res, next) => {
  csrfProtection(req, res, () => {
    try {
      ...
      res.status(500).json({ success: false, message: 'CSRF令牌获取失败' });
    }
  });
});
...
app.get('/api/api-tokens', async (req, res) => {
  ...
  try {
    const tokens = await ApiToken.getUserTokens(req.user.id);
    res.json({ success: true, tokens });
  } catch (err) {
    res.status(500).json({ success: false, message: '获取Token失败' });
  }
});

app.post('/api/api-tokens', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated()) return res.status(401).json({ success: false, message: '未登录' });
  const { description, expired_at } = req.body;
  if (!description) return res.status(400).json({ success: false, message: '描述不能为空' });
  try {
    // 只允许后端生成Token
    const result = await ApiToken.createToken({
      user_id: req.user.id,
      description,
      expired_at: expired_at ? new Date(expired_at) : null
    });
    res.json({ success: true, token: result.token }); // 只在创建时返回完整token
  } catch (err) {
    res.status(500).json({ success: false, message: '创建Token失败' });
  }
});

app.delete('/api/api-tokens/:token', csrfProtection, async (req, res) => {
  if (!req.isAuthenticated()) return res.status(401).json({ success: false, message: '未登录' });
  const { token } = req.params;
  try {
    await ApiToken.deactivateToken(token, req.user.id);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ success: false, message: '失效Token失败' });
  }
});
// ========== END API Token 管理接口 ==========
...
app.get('/api/members', async (req, res) => {
  try {
    ...
    if (req.query.autoPageSize === '1') {
      ...
      // 递归终止条件Ncrit: 若连续3次pageSize变化小于2或达到最大递归深度则终止
      let prevPageSize = pageSize;
      let ncrit = 0;
      let maxDepth = 5;
      for (let i = 0; i < maxDepth; i++) {
        let newPageSize = Math.round(10 + entropy * 10 + i * 2);
        newPageSize = Math.min(Math.max(1, newPageSize), 100);
        if (Math.abs(newPageSize - prevPageSize) < 2) {
          ncrit++;
        } else {
          ncrit = 0;
        }
    ...
    res.json({ success: true, members, total, pageSize }); // 返回建议分页粒度
  } catch (err) {
...
app.get('/api/chat/messages', async (req, res) => {
  ...
  const { targetType, targetId, limit, beforeId, autoPageSize } = req.query;
  let sql = '';
  let params = [];
  let pageLimit = Math.min(Number(limit) || 50, 100);

  // 信息熵自适应分页（可选）
  if (autoPageSize === '1') {
    const { rows: allRows } = await pool.query(
      targetType === 'public'
        ? `SELECT EXTRACT(EPOCH FROM (NOW() - created_at)) AS age FROM chat_messages WHERE target_type='public'`
        : `SELECT EXTRACT(EPOCH FROM (NOW() - created_at)) AS age FROM chat_messages WHERE target_type='private' AND (sender_id=$1 OR target_id=$1)`,
      targetType === 'public' ? [] : [req.user.id]
    );
    const ages = allRows.map(r => Number(r.age));
    const sum = ages.reduce((a, b) => a + b, 0) || 1;
    const probs = ages.map(x => x / sum);
    const entropy = -probs.reduce((s, p) => (p > 0 ? s + p * Math.log(p) : s), 0);
    let prevLimit = pageLimit;
    let ncrit = 0;
    let maxDepth = 5;
    for (let i = 0; i < maxDepth; i++) {
      let newLimit = Math.round(20 + entropy * 10 + i * 2);
      newLimit = Math.min(Math.max(1, newLimit), 100);
      if (Math.abs(newLimit - prevLimit) < 2) {
        ncrit++;
      } else {
        ncrit = 0;
      }
  ...
  if (targetType === 'public') {
    ...
    if (beforeId) {
      sql = `SELECT m.*, u.username, u.is_admin, u.is_super_admin 
             ...
             ORDER BY m.id DESC 
             LIMIT $4`;
      params = [req.user.id, Number(targetId), beforeId, pageLimit];
    } else {
      sql = `SELECT m.*, u.username, u.is_admin, u.is_super_admin 
             FROM chat_messages m 
             JOIN users u ON m.sender_id = u.id 
             WHERE m.target_type = 'private' 
               AND ((m.sender_id = $1 AND m.target_id = $2) OR (m.sender_id = $2 && m.target_id = $1))
             ORDER BY m.id DESC 
             LIMIT $3`;
      params = [req.user.id, Number(targetId), pageLimit];
    }
  } else {
  ...
  try {
    const { rows } = await pool.query(sql, params);
    rows.reverse();
    res.json({ success: true, messages: rows, pageLimit });
  } catch (err) {
...
app.post('/api/admin/kick-user', csrfProtection, requireSuperAdminOrAdmin, async (req, res) => {
  ...
  try {
    ...
    await pool.query('DELETE FROM user_friends WHERE user_id=$1 OR friend_id=$1', [userId]);
    await pool.query('DELETE FROM chat_messages WHERE sender_id=$1 OR target_id=$1', [userId]);
    await pool.query('DELETE FROM api_tokens WHERE user_id=$1', [userId]);
    await pool.query('DELETE FROM user_unread_chat WHERE user_id=$1 OR target_id=$1', [userId]);
    await pool.query('DELETE FROM operation_logs WHERE user_id=$1 OR target_id=$1', [userId]);
 ...
 * form-data: file
 * 返回: { success: true, url: '/uploads/xxx.png' }
 */
app.post('/api/upload-image', upload.single('file'), async (req, res) => {
  try {
    ...
    await logOperation({
      ...
      ip: req.ip,
      channel_mode: 'user_upload'
    });
    // 返回图片URL
    const url = '/uploads/' + req.file.filename;
    res.json({ success: true, url });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message || '图片上传失败' });
  }
});
// ========== END 图片上传接口 ==========
...
io.on('connection', (socket) => {
  ...
  socket.on('chat message', async (msg) => {
    ...
    const data = {
      ...
      id: socket.user.id,
      senderId: socket.user.id, // 新增字段，兼容前端判断
...
server.listen(PORT, async () => {
  try {
    ...
    console.log(`CORS允许的源: https://nenwa618.github.io, http://localhost:3000, http://localhost:5500, https://qianxian-backend.onrender.com`);
  } catch (err) {
...
// 注意：只保留一处 const server = http.createServer(app); 并用 server.listen 启动服务。
// 如果你在文件中还有其它 const server = http.createServer(app);，请删除多余的声明。