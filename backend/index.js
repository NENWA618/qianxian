...
async function getSiteContentWithEntropy() {
  siteContentCache.accessCount++;
  // 计算概率和熵
  const total = siteContentCache.accessCount;
  const p = 1 / total;
  const entropy = -p * Math.log(p);
  // 高熵时强制刷新缓存
  if (
    !siteContentCache.data ||
    entropy > 0.5 ||
    Date.now() - siteContentCache.lastUpdate > 1000 * 60 * 5
  ) {
    ...
    siteContentCache.lastUpdate = Date.now();
    siteContentCache.lastEntropy = entropy;
  }
