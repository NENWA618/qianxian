...
const sessionConfig = {
  ...
  cookie: {
    ...
    domain: process.env.NODE_ENV === 'production' ? '.qianxian-backend.onrender.com' : undefined
  },
...
app.get('/api/csrf-token', (req, res) => {
  try {
    ...
    const token = req.csrfToken(); // 现在可以安全调用
    res.json({ 
      token,
      expires: new Date(Date.now() + 24 * 60 * 60 * 1000)
    });
  } catch (err) {
    console.error('CSRF令牌生成失败:', err);
    res.status(500).json({ 
      success: false,
      message: '无法生成CSRF令牌',
...
app.post('/api/login', csrfProtection, (req, res, next) => {
  ...
  console.log('收到的CSRF令牌:', req.headers['x-csrf-token'] || req.body._csrf);
