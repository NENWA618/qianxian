 ...
 * 新增：支持多维同步性与递归熵判据相关日志（论文算法应用点）
 * 新增：支持参数变更、异常检测、监控指标等系统级事件日志
 * 优化：多维判据、递归熵、分形维度、Langevin噪声等指标结构化记录
 * 2025-06-26 优化：支持更多结构化指标、实验参数、EEG映射、βcrit/βexit、Ncrit、Banach、分形维度等日志
 */
 ...
 * - action: 操作类型（如 login、register、edit_content、set_admin、rate_limit、cache_hit、cache_miss、batch_task、param_update、param_reset、monitor_event、eeg_upload、experiment_param_update、beta_crit_trigger、beta_exit_recover、ncrit_terminate、banach_converge、fractal_dimension 等）
 * - detail: 操作详情（JSON字符串或文本，支持多维判据、递归熵、分形维度、βcrit/βexit、Ncrit、Banach等结构化数据）
 * - ip: 操作IP
...
/**
 * 通用操作日志写入函数
 * 支持多维同步性与递归熵判据相关日志（论文算法应用点）
 * 支持参数变更、异常检测、监控指标、实验参数、EEG映射、βcrit/βexit、Ncrit、Banach、分形维度等系统级事件日志
 * @param {object} param0
...
async function logOperation({
  ...
  try {
    // 多维同步性/递归熵/分形维度/实验参数/EEG映射/βcrit/βexit/Ncrit/Banach等结构化指标自动JSON化
    ...
    if (
      ...
      (
        ...
        action === "param_update" ||
        action === "param_reset" ||
        action === "experiment_param_update" ||
        action === "eeg_upload" ||
        action === "beta_crit_trigger" ||
        action === "beta_exit_recover" ||
        action === "ncrit_terminate" ||
        action === "banach_converge" ||
    ...
    // 敏感信息自动脱敏
    safeDetail = maskSensitiveDetail(action, safeDetail);
