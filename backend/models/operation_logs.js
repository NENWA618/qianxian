 ...
 * 2025-06-26 优化：支持更多结构化指标、实验参数、EEG映射等日志
 */
 ...
 * - action: 操作类型（如 login、register、edit_content、set_admin、rate_limit、cache_hit、cache_miss、batch_task、param_update、param_reset、monitor_event、eeg_upload、experiment_param_update 等）
 * - detail: 操作详情（JSON字符串或文本，支持多维判据、递归熵、分形维度等结构化数据）
 * - ip: 操作IP
 ...
 * 支持参数变更、异常检测、监控指标、实验参数、EEG映射等系统级事件日志
 * @param {object} param0
...
async function logOperation({
  ...
  try {
    // 多维同步性/递归熵/分形维度/实验参数/EEG映射等结构化指标自动JSON化
