 ...
 * 新增：所有Token相关操作自动写入操作日志，便于审计与监控
 */

const { DataTypes, Op } = require("sequelize");
const sequelize = require("../db");
const crypto = require("crypto");
const { logOperation } = require("./operation_logs");
...
async function createToken({ user_id, description, expired_at }) {
  ...
  await logOperation({
    ...
    detail: { token: maskToken(token), description, expired_at },
...
async function deactivateToken(token, user_id) {
  ...
  await logOperation({
    user_id,
    username: "system",
    action: "api_token_deactivate",
    detail: { token: maskToken(token) },
    channel_mode: "api_token"
  });
  return result;
}
